...
class DbMysql {
    ...
    extract() {
        return __awaiter(this, void 0, void 0, function* () {
            const tables = {};
            const data = yield this.connection.query('show tables');
            ...
            for (const tableName of Object.keys(tables)) {
                // get column list
                for (const row of yield this.connection.query(`DESCRIBE ${tableName}`)) {
                    let type = row['Type'];
                ...
                // get index list
                for (const row of yield this.connection.query(`SHOW INDEX FROM ${tableName} WHERE Key_name != 'PRIMARY'`)) {
                ...
                for (const row of yield this.connection.query(query, [this.dbHost.database, tableName])) {
    ...
    static createAlterForeignKey(table, column, fk, onupdate, ondelete, tables) {
